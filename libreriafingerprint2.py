...
class PyFingerprint(object):
    ...
    def __readPacket(self, timeout=5.0):
        """
        ...
        i = 0
        print("Iniciando lectura de paquete...")
        while ( True ):
            ...
            if (receivedFragment is not None and len(receivedFragment) != 0):
                print("timeout de lectura alcanzado")
                receivedFragment = self.__stringToByte(receivedFragment)
                print(f"Byte {i}: {hex(receivedFragment)}")
            ...
            if ( i >= 12 ):
                ...
                print("Lectura de paquete mínimo completa")
                ...
                if ( receivedPacketData[0] != self.__rightShift(FINGERPRINT_STARTCODE, 8) or receivedPacketData[1] != self.__rightShift(FINGERPRINT_STARTCODE, 0) ):
                    print("Cabecera de paquete inválida")
                ...
                print(f"Longitud del payload del paquete: {packetPayloadLength}")
                ...
                if ( i < packetPayloadLength + 9 ):
                    print("Esperando datos adicionales del paquete...")
                    continue
                ...
                packetChecksum = (packetType + receivedPacketData[7] + receivedPacketData[8]) % 65536

                packetPayload = []
                
                print(f"Iniciando bucle for con rango: 9 a {9 + packetPayloadLength - 2}")
                if packetPayloadLength < 2:
                    print("Error: packetPayloadLength es demasiado corto")
                    continue
                ...
                for j in range(9, 9 + packetPayloadLength - 2):
                    ...
                    packetChecksum = (packetChecksum + receivedPacketData[j]) % 65536
                    print(f"Agregando byte {j - 9 + 1}/{packetPayloadLength - 2}: {hex(receivedPacketData[j])} al checksum, total ahora: {hex(packetChecksum)}")
                    
                ## Calculate full checksum of the 2 separate checksum bytes
                receivedChecksumHigh = receivedPacketData[i - 2]
                receivedChecksumLow = receivedPacketData[i - 1]
                print(f"Checksum alto: {hex(receivedChecksumHigh)}, Checksum bajo: {hex(receivedChecksumLow)}")
                
                receivedChecksum = (self.__leftShift(receivedChecksumHigh, 8) | receivedChecksumLow)
                print(f"Checksum calculado: {hex(packetChecksum)}")
                print(f"Checksum recibido: {hex(receivedChecksum)}")
              

                if ( receivedChecksum != packetChecksum ):
                    print(f"Paquete recibido completo: {[hex(x) for x in receivedPacketData]}")
    ...
    def isTemplateStored(self, position):
        """
        Check if a template is stored at the given position.

        Arguments:
        position (int): The position to check.

        Returns:
        True if the template is stored, False otherwise.
        ...
        if position < 0 or position >= self.getStorageCapacity():
            raise ValueError('The given position number is invalid!')

    # Get the template index for the specific page
        templateIndex = self.getTemplateIndex(position // 8)  # Assuming each page has 8 templates

    # Check if the position is used
        return templateIndex[position % 8]  # Check the specific bit in the byte
    ...
    def createTemplate(self):
        """
        Combines the characteristics stored in char buffer 1 and char buffer 2 into one template.
    ...
    # Create a packet payload without parameters
        packetPayload = (
    ...
    # Check the received payload for success
        if receivedPacketPayload[0] == FINGERPRINT_OK:
            return True
        elif receivedPacketPayload[0] == FINGERPRINT_ERROR_COMMUNICATION:
            raise Exception('Communication error')
    ...
    def storeTemplate(self, positionNumber=-1, charBufferNumber=FINGERPRINT_CHARBUFFER1):
        ...
        Arguments:
        positionNumber (int): The position to store the template. If -1, finds a free position automatically.
        charBufferNumber (int): The char buffer to use (default is FINGERPRINT_CHARBUFFER1).
    
        Returns:
        The position where the template was stored.
    
        Raises:
        ValueError: if positionNumber or charBufferNumber is invalid.
        Exception: for other sensor communication errors.
        """
    # Encuentra un índice libre si no se especifica una posición
        if positionNumber == -1:
            for page in range(0, 4):  # Suponiendo que haya 4 páginas
                templateIndex = self.getTemplateIndex(page)
            
            # Verifica que templateIndex sea una lista para evitar errores de comparación
                if not isinstance(templateIndex, list):
                    raise TypeError("Expected templateIndex to be a list but got {}".format(type(templateIndex)))

            # Busca en la lista una posición libre
                for i in range(len(templateIndex)):
                    if not templateIndex[i]:  # Posición no ocupada
                        positionNumber = (page * 8) + i  # Asumiendo 8 posiciones por página
                        break

            # Si se encontró una posición, salir del bucle
                if positionNumber != -1:
                    break

    # Validación de la posición calculada o proporcionada
        if positionNumber < 0 or positionNumber >= self.getStorageCapacity():
            raise ValueError('The given position number is invalid!')

    # Validación del buffer de características
        if charBufferNumber not in [FINGERPRINT_CHARBUFFER1, FINGERPRINT_CHARBUFFER2]:
            raise ValueError('The given char buffer number is invalid!')

    # Crea el payload para almacenar la plantilla
        packetPayload = (
    ...
    # Verifica si el paquete es un ACK
        if receivedPacketType != FINGERPRINT_ACKPACKET:
            raise Exception('The received packet is not an ack packet!')
